    ...
    private fun updateRemoteValues() {
        ...
        sharedPreferenceUtils.apply {
            try {
                ...
                Log.i(
                    TAG_REMOTE, "RemoteConfiguration: rcAppOpenSplash -> ${
                        remoteConfig[appOpenSplash].asLong().toInt()
                    }"
                )

                Log.i(
                    TAG_REMOTE, "RemoteConfiguration: rcBannerHome -> ${
                        remoteConfig[bannerHome].asLong().toInt()
                    }"
                )

                Log.i(
                    TAG_REMOTE, "RemoteConfiguration: rcInterOnBoarding -> ${
                        remoteConfig[interOnBoarding].asLong().toInt()
                    }"
                )
                Log.i(
                    TAG_REMOTE, "RemoteConfiguration: rcInterFeature -> ${
                        remoteConfig[interFeature].asLong().toInt()
                    }"
                )

                Log.i(
                    TAG_REMOTE, "RemoteConfiguration: rcRewardedAiFeature -> ${
                        remoteConfig[rewardedAiFeature].asLong().toInt()
                    }"
                )
                Log.i(
                    TAG_REMOTE, "RemoteConfiguration: rcRewardedInterAiFeature -> ${
                        remoteConfig[rewardedInterAiFeature].asLong().toInt()
                    }"
                )

                Log.i(
                    TAG_REMOTE, "RemoteConfiguration: rcNativeLanguage -> ${
                        remoteConfig[nativeLanguage].asLong().toInt()
                    }"
                )
                Log.i(
                    TAG_REMOTE, "RemoteConfiguration: rcNativeOnBoarding -> ${
                        remoteConfig[nativeOnBoarding].asLong().toInt()
                    }"
                )
                Log.i(
                    TAG_REMOTE, "RemoteConfiguration: rcNativeHome -> ${
                        remoteConfig[nativeHome].asLong().toInt()
                    }"
                )
                Log.i(
                    TAG_REMOTE, "RemoteConfiguration: rcNativeFeature -> ${
                        remoteConfig[nativeFeature].asLong().toInt()
                    }"
                )
                Log.i(
                    TAG_REMOTE, "RemoteConfiguration: rcNativeExit -> ${
                        remoteConfig[nativeExit].asLong().toInt()
                    }"
                )
            } catch (ex: Exception) {
